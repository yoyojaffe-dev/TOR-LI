...
### A3. User Profile
- **My Bookings:** Tracking upcoming appointments with a gold countdown for the next visit.
...
## 7. Technical API Specifications

### 7.1 Barbershop Sync API
- **Webhooks:** Push notifications from barber management systems for real-time inventory updates.
- **Polling Fallback:** Scheduled synchronization every 60 seconds for systems without webhook support.
- **Conflict Resolution:** Last-write-wins logic with priority given to the barber's native system.

### 7.2 Booking Engine API
- **Slot Locking (POST /bookings/lock):** Atomic lock on a specific time slot with a 5-minute TTL (Time-To-Live). Returns a `lock_id`.
- **Confirmation (POST /bookings/confirm):** Finalizes the booking using the `lock_id` after successful payment.
- **Cancellation (DELETE /bookings/{id}):** Triggers cancellation logic, fee calculation, and releases the slot.

### 7.3 Payment & Escrow API
- **Apple Pay Integration:** Direct decryption of Apple Pay tokens for server-side processing.
- **Escrow Logic:** Holds no-show fees in a pending state until 2 hours before the appointment.
- **Payouts:** Weekly settlement to barbers via local bank transfer API (Bit/Pepper Pay supported).

### 7.4 Localization & Directionality
- **Translation API:** Dynamic dictionary fetching for Hebrew/English content.
- **RTL Transformer:** Middleware to flip layout direction and adjust directional icons (chevrons, pins) based on the active locale.
